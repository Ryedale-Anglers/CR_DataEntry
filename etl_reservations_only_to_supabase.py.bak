...
def deduplicate_catch_returns(conn):
    ...
    Note: the web form's insert_catch_return function now uses ON CONFLICT DO UPDATE,
    so duplicates from the web path should not accumulate. This function is retained
    as a safety net for historical data and any records loaded during the transition
    from Google Sheets ingestion.
    """
    try:
        schema = "private"
        dedup_sql = text(f"""
...
if __name__ == "__main__":
    try:
        ...
        print(" ")
        print("===============================================")
        print(f"Starting ETL process for Reservations at {datestamp}")
        ...
        if downloaded_data:
            with engine.begin() as conn:
                ...
                print(f"Starting - match_and_update_reservation_names")
